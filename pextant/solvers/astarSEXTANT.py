...
def fullSearch(waypoints, env_model, cost_function):
    from astar import aStarSearch

    fullpath = []
    fullcost = 0
    itemssrchd = 0
    for i in range(len(waypoints.coords) - 1):
        segmentCost = 0
        ...
        out = aStarSearch(node1, node2, cost_function)
        fullpath += out[0] +[node2.state]
        fullcost += out[2]
        itemssrchd += out[1]
    return fullpath, fullcost, itemssrchd
...
if __name__ == '__main__':
    ...
    out = fullSearch(waypoints, cost_function)
    print out